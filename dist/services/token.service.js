...
const generateAuthTokens = (user) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const accessToken = generateToken(user.id, accessTokenExpires, tokens_1.tokenTypes.ACCESS);
    const refreshTokenExpires = (0, moment_1.default)().add(config_1.config.jwt.refreshExpirationDays, 'days');
    const refreshToken = generateToken(user.id, refreshTokenExpires, tokens_1.tokenTypes.REFRESH);
    yield saveToken(refreshToken, user.id, refreshTokenExpires, tokens_1.tokenTypes.REFRESH);
    return {
        access: {
            token: accessToken,
...
const generateVerifyEmailToken = (user) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const verifyEmailToken = generateToken(user.id, expires, tokens_1.tokenTypes.VERIFY_EMAIL);
    yield saveToken(verifyEmailToken, user.id, expires, tokens_1.tokenTypes.VERIFY_EMAIL);
